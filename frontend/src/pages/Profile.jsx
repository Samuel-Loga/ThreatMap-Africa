...
<<<<<<< HEAD
  async function fetchSubmissions(userId) {
    ...
    try {
      ...
      setSubLoading(false)
=======
  async function fetchMyIndicators() {
    setIndicatorsLoading(true)
    try {
      const res = await indicatorsApi.list({ submitted_by: profile.id, limit: 10 })
      setMyIndicators(res.data)
    } catch (err) {
      console.error('Failed to load shared indicators', err)
    } finally {
      setIndicatorsLoading(false)
>>>>>>> 5dad0caca8e52e29fce4ce78799408c0d2ea4459
    }
  }
  ...
  return (
    <div className="space-y-6 pb-20">
        ...
        <div className="flex gap-3">
          {isEditing ? (
            ...
            <button 
              ...
              className="px-6 py-2 bg-primary hover:bg-primary/90 text-white text-xs font-black uppercase tracking-widest rounded-lg transition-all shadow-lg shadow-primary/20"
            >
              Edit Profile
            </button>
          )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        {/* Left Column: Institutional Context & Shared Indicators */}
          ...
          <section className="bg-dark-800 border border-dark-600 rounded-lg p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
              <div className="space-y-1">
                ...
                {isEditing ? (
                  <input className={inputClass} value={editData.organization || ''} onChange={e => setEditData({...editData, organization: e.target.value})} />
                ) : (
              ...
              <div className="space-y-1">
                ...
                {isEditing ? (
                  <input className={inputClass} value={editData.department || ''} onChange={e => setEditData({...editData, department: e.target.value})} />
                ) : (
              ...
              <div className="space-y-1">
                ...
                {isEditing ? (
                  <input className={inputClass} value={editData.city || ''} onChange={e => setEditData({...editData, city: e.target.value})} />
                ) : (
              ...
              <div className="space-y-1">
                ...
                {isEditing ? (
                  <input className={inputClass} value={editData.region_state || ''} onChange={e => setEditData({...editData, region_state: e.target.value})} />
                ) : (
          ...
          <section className="bg-dark-800 border border-dark-600 rounded-lg p-6 space-y-4">
            ...
            <div className="space-y-2 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
              {indicatorsLoading ? (
                ...
                <div className="bg-dark-900/30 border border-dark-700 border-dashed rounded-lg p-8 text-center">
                  <p className="text-gray-500 text-sm italic">You haven't shared any indicators yet.</p>
                  <Link to="/submit" className="text-primary text-xs font-bold mt-2 inline-block uppercase tracking-wider hover:underline">
        ...
        {/* Right Column: Interests, Preferences & Security */}
            ...
            <div className="flex flex-wrap gap-2">
              {isEditing ? (
                ['Phishing', 'Malware', 'DDoS', 'Ransomware', 'Social Engineering', 'APT', 'Cloud Security'].map(interest => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${
                      (editData.interests || []).includes(interest) ? 'bg-primary border-primary text-white' : 'bg-dark-700 border-dark-600 text-gray-400 hover:border-gray-500'
                    }`}
                  >
                    {interest}
                  </button>
                ...
                profile.interests?.length > 0 ? profile.interests.map(i => (
                  <span key={i} className="px-3 py-1 bg-dark-700 text-gray-200 text-xs font-bold rounded border border-dark-600">{i}</span>
                )) : <p className="text-gray-500 text-sm italic">No interests specified.</p>
              )}
            </div>
          </section>

          <section className="bg-dark-800 border border-dark-600 rounded-lg p-6 space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between text-sm">
                ...
                {isEditing ? (
                  <input type="checkbox" checked={editData.email_notif} onChange={e => setEditData({...editData, email_notif: e.target.checked})} />
                ) : (
            ...
            <div className="pt-6 border-t border-dark-700 space-y-4">
              <h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest">Secure Communication</h3>
              {isEditing ? (
                ...
                <div className="bg-dark-900 p-3 rounded border border-dark-700">
                  <pre className="text-[9px] text-gray-500 font-mono whitespace-pre-wrap overflow-x-auto max-h-24">
